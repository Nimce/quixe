...
current_metrics=res;create_resize_sensors();detect_external_links=iface.detect_external_links;if(detect_external_links){regex_external_links=iface.regex_external_links;if(!regex_external_links){if(detect_external_links=='search'){regex_external_links=RegExp('\\b((?:https?://)(?:[^\\s()<>]+|\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\))+(?:\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\)|[^\\s`!()\\[\\]{};:\'".,<>?\u00ab\u00bb\u201c\u201d\u2018\u2019]))','i');}else{regex_external_links=RegExp('^https?:','i');}}}
...
if(recording){var qparams=get_query_params();var flag=qparams['feedback'];if(jQuery.type(flag)!='undefined'&&flag!='1'){recording=false;glkote_log('User has opted out of transcript recording.');}else{recording_state={sessionId:(new Date().getTime())+""+(Math.ceil(Math.random()*10000)),input:null,output:null,timestamp:0,outtimestamp:0}
...
debugmod=iface.debug_commands;if(!debugmod){glkote_log('The debug_commands option is set, but there is no GiDebug module.');}else{debugmod.init(evhan_debug_command);debug_out_handler=debugmod.output;if(iface.debug_console_open)
debugmod.open();}}
if(!iface.font_load_delay){send_response('init',null,current_metrics);}else{disabled=true;defer_func(function(){disabled=false;current_metrics=measure_window();send_response('init',null,current_metrics);});}}
...
if(arg.type=='retry'){if(!retry_timer){glkote_log('Event has timed out; will retry...');show_loading();retry_timer=delay_func(2,retry_update);}else{glkote_log('Event has timed out, but a retry is already queued!');}
return;}
...
accept_specialinput(arg.specialinput);jQuery.each(windowdic,function(winid,win){if(win.type=='buffer'&&win.needscroll){win.needscroll=false;if(!win.needspaging){var frameel=win.frameel;if(!perform_paging){frameel.scrollTop(frameel.get(0).scrollHeight);win.needspaging=false;}else{frameel.scrollTop(win.topunseen-current_metrics.buffercharheight);win.pagefrommark=win.topunseen;var frameheight=frameel.outerHeight();var realbottom=buffer_last_line_top_offset(win);var newtopunseen=frameel.scrollTop()+frameheight;if(newtopunseen>realbottom)
newtopunseen=realbottom;if(win.topunseen<newtopunseen)
win.topunseen=newtopunseen;if(frameel.scrollTop()+frameheight+moreprompt_margin>=frameel.get(0).scrollHeight){win.needspaging=false;}else{win.needspaging=true;}}
...
prevel.remove();}else{if(!moreel.length){moreel=$('<div>',{id:'win'+win.id+'_moreprompt','class':'MorePrompt'});moreel.append('More');var morex=win.coords.right+approx_scroll_width;var morey=win.coords.bottom;moreel.css({bottom:morey+'px',right:morex+'px'});$('#'+windowport_id,dom_context).append(moreel);}
...
if(!content||!content.length){lineel.text(NBSP);}else{lineel.empty();for(sx=0;sx<content.length;sx++){var rdesc=content[sx];var rstyle,rtext,rlink;if(jQuery.type(rdesc)==='object'){if(rdesc.special!==undefined)
continue;rstyle=rdesc.style;rtext=rdesc.text;rlink=rdesc.hyperlink;}else{rstyle=rdesc;sx++;rtext=content[sx];rlink=undefined;}
var el=$('<span>',{'class':'Style_'+rstyle});if(rlink==undefined){insert_text_detecting(el,rtext);}else{var ael=$('<a>',{'href':'#','class':'Internal'});ael.text(rtext);ael.on('click',build_evhan_hyperlink(win.id,rlink));el.append(ael);}
lineel.append(el);}}}}
...
rstyle=rdesc.style;rtext=rdesc.text;rlink=rdesc.hyperlink;}else{rstyle=rdesc;sx++;rtext=content[sx];rlink=undefined;}
var el=$('<span>',{'class':'Style_'+rstyle});if(rlink==undefined){insert_text_detecting(el,rtext);}else{var ael=$('<a>',{'href':'#','class':'Internal'});ael.text(rtext);ael.on('click',build_evhan_hyperlink(win.id,rlink));el.append(ael);}
divel.append(el);}}
...
maxlen=argi.maxlen;var newinputel=false;var inputel=win.inputel;if(inputel==null){newinputel=true;var classes='Input';if(argi.type=='line'){classes+=' LineInput';}else if(argi.type=='char'){classes+=' CharInput';}else{glkote_error('Window '+win.id+' has requested unrecognized input type '+argi.type+'.');}
...
win.terminators[argi.terminators[ix]]=true;}}else if(argi.type=='char'){inputel.on('keypress',evhan_input_char_keypress);inputel.on('keydown',evhan_input_char_keydown);}
...
if(!arg){}else{request_timer_interval=arg;request_timer=window.setTimeout(evhan_timer_event,request_timer_interval);}}
function accept_specialinput(arg){if(arg.type=='fileref_prompt'){var replyfunc=function(ref){send_response('specialresponse',null,'fileref_prompt',ref);};try{var writable=(arg.filemode!='read');Dialog.open(writable,arg.filetype,arg.gameid,replyfunc);}catch(ex){GlkOte.log('Unable to open file dialog: '+ex);replyfunc=function(ref){send_response('specialresponse',null,'fileref_prompt',null);};defer_func(replyfunc);}}else{glkote_error('Request for unknown special input type: '+arg.type);}}
...
if(detect_external_links=='match'){if(regex_external_links.test(val)){var ael=$('<a>',{'href':val,'class':'External','target':'_blank'});ael.text(val);el.append(ael);return;}}else if(detect_external_links=='search'){while(true){var match=regex_external_links.exec(val);if(!match)
...
ctx.fillStyle=op.color;if(op.x===undefined){ctx.fillRect(0,0,win.graphwidth,win.graphheight);win.frameel.css('background-color',ctx.fillStyle);}else{ctx.fillRect(op.x,op.y,op.width,op.height);}
ctx.fillStyle='#000000';break;case'image':if(!loadedimg){var oldimg=image_cache[op.image];if(oldimg&&oldimg.width>0&&oldimg.height>0){loadedimg=oldimg;loadedev=true;}else{delete image_cache[op.image];}}
...
function delay_func(timeout,func){return window.setTimeout(func,timeout*1000);}
function defer_func(func){return window.setTimeout(func,0.01*1000);}
...
TabMode.reset(val);send_response('line',win,val,termkey);}
...
res.terminator=val2;}else if(type=='char'){res.window=win.id;res.value=val;}else if(type=='hyperlink'){res.window=win.id;res.value=val;}else if(type=='mouse'){res.window=win.id;res.x=val;res.y=val2;}else if(type=='external'){res.value=val;}else if(type=='specialresponse'){res.response=val;res.value=val2;}else if(type=='debuginput'){res.value=val;}else if(type=='redraw'){res.window=win.id;}else if(type=='init'){res.metrics=val;res.support=['timer','graphics','graphicswin','hyperlinks'];}else if(type=='arrange'){res.metrics=val;}
...
inputtype=input.type;if(inputtype=='line'||inputtype=='char'){recording_state.input=input.value;}else if(inputtype=='init'||inputtype=='external'||inputtype=='specialresponse'||!inputtype){recording_state.input='';}else{send=false;}
...
buffer=buffer+'\n';if(text.content){for(var kx=0;kx<text.content.length;kx++){var el=text.content[kx];if(jQuery.type(el)=='string'){kx++;buffer=buffer+text.content[kx];}else{if(el.text)
...
ev.preventDefault();return;}}else{var res=null;if(keycode==13)
...
ypos=0;}else if(win.type=='graphics'){var canel=$('#win'+win.id+'_canvas',dom_context);if(canel.length){var pos=canel.offset();xpos=ev.clientX-pos.left;ypos=ev.clientY-pos.top;}
...
return true;send_response('char',win,res);return false;}
TabMode={enabled:false,tabModePrevHistoryValues:[],tabModePrefix:'',matchedWords:[],getWords:function(){var words=[];jQuery('.Style_normal, .Style_input').each(function(){let text=jQuery(this).text();let matches=text.match(/(^|[\s\"\'])([a-z_0-9]{2,})(?=$|[\s\"\'\,\.])/ig);for(var i in matches){let word=matches[i].trim();word=word.replace(/[\"\'\.]/g,'');words.push(word);}});return TabMode.matchedWords.concat(words.filter(function(value,index,self){return self.indexOf(value)===index;}));},addToMatchedWords:function(word){console.log('added to matched words',word);let matchedWordsRev=TabMode.matchedWords.reverse();matchedWordsRev.push(word.trim());TabMode.matchedWords=matchedWordsRev.reverse();},reset:function(currentValue){console.log('Tab reset called',currentValue);TabMode.enabled=false;TabMode.tabModePrevHistoryValues=[];TabMode.tabModePrefix=false;if(currentValue!==false&&currentValue!=''){const val=currentValue.trim();const lastSpace=val.lastIndexOf(" ");var lastWord=val;if(lastSpace!=-1){lastWord=val.substring(lastSpace+1);}
if(lastWord!=''){TabMode.addToMatchedWords(lastWord);}}},enable:function(prefix){TabMode.reset(false);TabMode.enabled=true;TabMode.tabModePrefix=prefix;},getNextSuggestion:function(){const words=TabMode.getWords();for(var i in words){if(words[i].startsWith(TabMode.tabModePrefix)&&TabMode.tabModePrevHistoryValues.indexOf(words[i])==-1){TabMode.tabModePrevHistoryValues.push(words[i]);return words[i];}}
TabMode.tabModePrevHistoryValues=[];for(var i in words){if(words[i].startsWith(TabMode.tabModePrefix)&&TabMode.tabModePrevHistoryValues.indexOf(words[i])==-1){TabMode.tabModePrevHistoryValues.push(words[i]);return words[i];}}
return'';}};function evhan_input_keydown(ev){var keycode=0;if(ev)keycode=ev.keyCode;if(!keycode)return true;if(keycode==key_codes.KEY_TAB){var winid=$(this).data('winid');var win=windowdic[winid];if(!win||!win.input)
return true;if(TabMode.enabled){const val=this.value.trim();const lastSpace=val.lastIndexOf(" ");var valTrimmed='';if(lastSpace!=-1){valTrimmed=val.substring(0,lastSpace+1);}
const nextSuggestion=TabMode.getNextSuggestion();if(nextSuggestion!=''){this.value=valTrimmed+nextSuggestion;}}else{const val=this.value.trim();const lastSpace=val.lastIndexOf(" ");var lastWord=val;var valTrimmed='';if(lastSpace!=-1){lastWord=val.substring(lastSpace+1);valTrimmed=val.substring(0,lastSpace+1);}
if(lastWord!=''){TabMode.enable(lastWord);}
const nextSuggestion=TabMode.getNextSuggestion();if(nextSuggestion!=''){this.value=valTrimmed+nextSuggestion;}}
return false;}else if(keycode==key_codes.KEY_UP||keycode==key_codes.KEY_DOWN){var winid=$(this).data('winid');var win=windowdic[winid];if(!win||!win.input)
return true;TabMode.reset(false);if(keycode==key_codes.KEY_UP&&win.historypos>0){win.historypos-=1;if(win.historypos<win.history.length)
...
this.value=win.history[win.historypos];else
this.value='';}
return false;}else if(terminator_key_values[keycode]){var winid=$(this).data('winid');var win=windowdic[winid];if(!win||!win.input)
...
TabMode.reset(false);return true;}
